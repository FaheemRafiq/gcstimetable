...
export function AllocationCell({
  ...
  return (
    <span className={cn('text-sm', className)}>
      {allocation?.course?.code && <span>{allocation?.course?.display_code}</span>}
      {allocation?.teacher?.name && <span> - {allocation?.teacher?.name}</span>}
      ...
      {allocation.day?.name && (
        <>
          ...
          <Badge
            asChild
            className={cn('pointer-events-none', getBackgroundColor(allocation.day?.name))}
          >
