...
export function UserActions({ row }: { row: UserType }) {
  ...
  function handleEditPage(row: UserType) {
    ...
    <Fragment>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            <DropdownMenuItem className="cursor-pointer" onClick={() => setShowView(true)}>
              <Eye className="mr-2 h-4 w-4" />
              <span>View</span>
            </DropdownMenuItem>

            <DropdownMenuItem className="cursor-pointer" onClick={() => handleEditPage(row)}>
      ...
      {/* Modals */}
      <ViewUserModal show={ShowView} handleClose={handleCloseView} row={row} />
      ...
      <DeleteConfirmationDialog
        ...
        message={
          <span>
            Once a user is deleted, it cannot be recovered. Are you sure you want to delete this
            user <strong>{row.email}</strong>?
          </span>
        }
