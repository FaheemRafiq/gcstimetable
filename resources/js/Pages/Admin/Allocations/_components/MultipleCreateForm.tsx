...
function MultipleCreateForm({ props }: MultipleCreateForm) {
    ...
    function handleOpenDelete() {
        ...
        if(data.allocations_data.length > 0) {
            let validAllocations = data.allocations_data.filter(allocation => allocation.allocation_id !== undefined && allocation.allocation_id !== null);
    ...
    const selectedDays = useMemo(() => {
        return props?.timetable?.shift?.institution?.days?.filter(day => data.selected_days.includes(day.id));
    }, [data.selected_days])
    ...
    function handleOnDayCardClick(day: Day, allocation: UnCommonProps | Allocation | undefined) {
        ...
        let allocations_data = alreadySelectedDay ? data.allocations_data.filter(allocation => allocation.day_id !== day.id) : [
        ...
        setData({
            ...
            selected_days: alreadySelectedDay ? data.selected_days.filter(id => id !== day.id) : [...data.selected_days, day.id]
        })
        clearErrors()
    }
                            ...
                            <ScrollArea className="h-[calc(100vh-20rem)]">
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 p-1">
                                    {props?.timetable?.shift?.institution?.days?.map(day => {
                                        ...
                                        const filledDay = data.allocations_data.find(allocation => allocation.day_id === day.id);
                                        let allocationUpdated = false;

                                        if (isSelected && allocation) {
                                            if (allocation.room_id !== filledDay?.room_id || allocation.teacher_id !== filledDay?.teacher_id || allocation.course_id !== filledDay?.course_id) {
                                        ...
                                        return (
                                            <DayCard
                                                ...
                                                onClick={() => {
                                                    handleOnDayCardClick(day, {
                                                        ...
                                                        teacher_id: allocationData?.teacher_id ?? allocation?.teacher_id ?? null,
                                            ...
                                            >
                                                <div className="pt-12 px-4 pb-4">
                                                    ...
                                                    ) : (filledDay?.course_id || filledDay?.teacher_id || filledDay?.room_id) ? (
                                                        <div className="space-y-2">
                                                            {filledDay?.course_id && (
                                                                <div className="flex items-center gap-2">
                                                                    <Book className="w-4 h-4 text-primary" />
                                                                    <span className='text-sm'>
                                                                        {
                                                                            props?.courses.find(course => course.id === filledDay.course_id)
                                                                                ?.display_code
                                                                        }
                                                                    </span>
                                                                </div>
