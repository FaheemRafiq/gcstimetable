  ...
  const TotalRecords: React.FC<{
    ...
    return (
      <>
        ...
        <span className="text-xs">{paramFrom && paramTo && `(${paramFrom} to ${paramTo})`}</span>
      </>
    )
  }
    ...
    <div>
          ...
          {finalProps.searchFilter ? (
            <div className="relative max-w-md w-full">
              <Input
                ...
                value={(table.getColumn(finalProps.filterColumn)?.getFilterValue() as string) ?? ''}
                onChange={event =>
                  finalProps.search === 'server'
                    ? finalProps.onSearch?.(event.target.value)
                    : table.getColumn(finalProps.filterColumn)?.setFilterValue(event.target.value)
                }
                className="md:shadow-md"
                autoFocus
              />
              {((table.getColumn(finalProps.filterColumn)?.getFilterValue() as string) ?? '') && (
                <button
      ...
      <div className="flex items-center justify-between space-x-2 py-4">
        <div className="self-start text-nowrap text-sm text-muted-foreground">
          <TotalRecords
            paramTotalCount={finalProps.pagination ? table.getRowCount() : (totalCount ?? 0)}
            paramFrom={
              finalProps.pagination ? pagination.pageIndex * pagination.pageSize + 1 : from
            }
            paramTo={
        ...
        {pageLinks && <TablePagination links={pageLinks} />}
      </div>
    </div>
  )
}
