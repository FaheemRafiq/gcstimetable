                ...
                <div className="flex flex-col w-16 border-r sticky left-0 bg-background z-10">
                    {timeSlots.map((slot) => (
                        <div
                            key={slot.minutes}
                            className={cn(
                                "text-xs font-medium flex items-center justify-center border-b"
                            )}
                ...
                {days.map((day) => (
                    <div key={day} className="flex-1 border-r relative">
                        ...
                        {events[day]?.map((event, index) => {
                            const startMinutes =
                                ensureDate(event.startTime).getHours() *
                                    OneHoursInMinutes +
                                ensureDate(event.startTime).getMinutes();
                            const endMinutes =
                                ensureDate(event.endTime).getHours() *
                                    OneHoursInMinutes +
                            ...
                            const startSlot =
                                (startMinutes -
                                    startsFrom * OneHoursInMinutes) /
                                timeSlot; // Map to 20-min slots
                            const duration =
                                (endMinutes - startMinutes) / timeSlot; // Convert to slots
